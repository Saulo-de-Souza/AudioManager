@tool
@icon("res://addons/audio_manager/icons/icon_plus.svg")

## AudioStreamPlus is a super node based on Godot's native AudioStreamPlayer node with improvements.
class_name AudioStreamPlus extends Node
...
## Retorna o tamanho do stream com os ajustes pitch scale ou se for use_clipper == true, o clipper recortado jun com pitch scale.
...
func _create_audio_stream_player() -> AudioStreamPlayer:
	...
	new_audio_stream_player.mix_target = mix_target
